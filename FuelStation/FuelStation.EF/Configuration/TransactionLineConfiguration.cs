...
internal class TransactionLineConfiguration : IEntityTypeConfiguration<TransactionLine>
{
    public void Configure(EntityTypeBuilder<TransactionLine> builder)
    {
        ...
        builder.Property(tLine => tLine.DiscountPercentage).HasColumnType("decimal(10,2)");
        ...
        builder.HasOne(tLine => tLine.Transaction).WithMany(transaction => transaction.TransactionLines).HasForeignKey(tLine => tLine.TransactionID).OnDelete(DeleteBehavior.Restrict);
        builder.HasOne(tLine => tLine.Item).WithMany(item => item.TransactionLines).HasForeignKey(transaction => transaction.ItemID).OnDelete(DeleteBehavior.Restrict);
    }
}